    ...
    document.addEventListener("DOMContentLoaded", function() {
            ...
            check.addEventListener("click", () => {
                ...
                if (count > row[6].innerHTML) {
                    ...
                    fila.innerHTML =
                        ...
                        count +
                        `</td>
                        <td class='txt-id text-center'>` +
                        idModal +
                        `</td>
                        <td class='txt-name text-center'>` +
                        nameModal +
                        `</td>
                    ...
                    check.checked = false;
                    //Actualizar el stock
                    stockModal = check.parentElement.parentElement.getElementsByTagName(
                        "td"
                    )[3];
                    ...
                    /* Events for row */
                    var subtotal_invoice = document.querySelector(
                        ".subtotal-invoice"
                    );
                    var total_invoice = document.querySelector(
                        ".total-invoice"
                    );
                    var totalSuma = 0;
                    var sumatoriaTotal = document.querySelectorAll(
                        ".txt-total"
                    );
        ...
        function buscarContactos(e) {
            ...
            registros.forEach(registro => {
                ...
                if (
                    registro.childNodes[8].textContent
                        .replace(/\s/g, " ")
                        .search(expresion) != -1
                ) {
...
$(document).ready(function() {
    ...
    btnSend.on("click", e => {
        ...
        if (
            !client ||
            parseFloat(totalPay) <= 0 ||
            isNaN(parseFloat(totalPay))
        ) {
